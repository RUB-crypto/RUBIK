...
# rubik_hash
RUN git clone https://github.com/rubikpay/rubik_hash
RUN cd rubik_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} rubik
RUN useradd -u ${USER_ID} -g rubik -s /bin/bash -m -d /rubik rubik
...
RUN mkdir /rubik-src && \
  ...
  chown $USER_ID:$GROUP_ID /rubik-src && \

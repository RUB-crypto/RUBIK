...
const char * const BITCOIN_CONF_FILENAME = "rubik.conf";
const char * const BITCOIN_PID_FILENAME = "rubikd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "rubik" is a composite category enabling all Rubik-related debug output
                if(ptrCategory->count(std::string("rubik"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\RubikCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\RubikCore
    // Mac: ~/Library/Application Support/RubikCore
    // Unix: ~/.rubikcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "RubikCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/RubikCore";
#else
    // Unix
    return pathRet / ".rubikcore";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty rubik.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override rubik.conf

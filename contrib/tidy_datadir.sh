...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Rubik database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Rubik datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Rubik datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Rubik 0.7 datadir."
    ;;
  3)
    echo "Detected Rubik pre-0.8 datadir."
    ;;
  4)
    echo "Detected Rubik 0.8 datadir."
    ;;
esac

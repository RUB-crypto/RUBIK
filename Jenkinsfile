...
// which are part of the Rubik repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="rubik-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /rubik-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "rubikpay-rubik/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from rubikpay-rubik/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'rubikpay-rubik/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from rubikpay-rubik/develop"
            }
          }

          if (hasCache) {
            sh "cd /rubik-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /rubik-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /rubik-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /rubik-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /rubik-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /rubik-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /rubik-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
